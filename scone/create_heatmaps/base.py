...
class CreateHeatmapsBase(abc.ABC):
    ...
    def load_data(self):
        ...
        if self.lcdata_path.endswith('csv'): #CCC 
            self.metadata, self.lcdata = pd.read_csv(self.metadata_path), pd.read_csv(self.lcdata_path)
            survey = 'LSST'
            number_to_passband = {
                0:'u',
                1:'g',
            ...
            self.lcdata['passband'] = self.lcdata['passband'].map(number_to_passband)
            self.lcdata = Table.from_pandas(self.lcdata)
        else:
        ...
        # FBB: after splitting the ids do not include type
        # modifying here what is read from the hdf5

        self.ids_notype = np.array([tid.split('_')[1] for tid in self.ids.astype(str)]).astype(int)

        self.ids_for_current_file = np.intersect1d(self.lcdata_ids, self.ids_notype) if self.has_ids else self.lcdata_ids
        ...
        for output_path, mjd_minmax in zip(output_paths, mjd_minmaxes):
            ...
            with tf.io.TFRecordWriter("{}/heatmaps_{}.tfrecord".format(output_path, self.index)) as writer:
                for i, sn_id in enumerate(self.ids_for_current_file):
                    print("{}/heatmaps_{}.tfrecord".format(output_path, self.index), sn_id)
                    if i % 1000 == 0:
                    ...
                    #FBB no z error in plasticc...
                    z_err = 0 #sn_metadata['true_z_err'].iloc[0]
    ...
    def _get_sn_data(self, sn_id, mjd_minmax):
        ...
        # FBB testingprint(self.metadata.object_id == sn_id)
        # FBB added casting of object_id to str cause of type inconsistency
